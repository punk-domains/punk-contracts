...
const royaltyFeeUpdater = "0xb29050965A5AC70ab487aa47546cdCBc97dAE45D";
...
async function main() {
  ...
  const instance = await contract.deploy(knsAddress, domainAddress, usdcAddress, price, royaltyFeeUpdater);
  
  console.log("Contract address:", instance.address);

  console.log("Wait a minute and then run this command to verify contracts on Etherscan:");
  console.log("npx hardhat verify --network " + network.name + " " + instance.address + " " + knsAddress + " " + domainAddress + " " + usdcAddress + ' "' + price + '" ' + royaltyFeeUpdater);
}
