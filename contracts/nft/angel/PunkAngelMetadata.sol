...
contract PunkAngelMetadata is Ownable {
  ...
  function getMetadata(
    ...
    string memory bg1 = getSlice(1, 6, features);
    string memory bg2 = getSlice(7, 12, features);
    string memory hair = getSlice(13, 18, features);
    string memory skin = getSlice(19, 24, features);
    string memory dress = getSlice(25, 30, features);

    string memory faceIndexStr = getSlice(31, 31, features);
    string memory armsIndexStr = getSlice(32, 32, features);
    string memory lipsIndexStr = getSlice(33, 33, features);
    ...
    return string(
      abi.encodePacked("data:application/json;base64,",Base64.encode(bytes(abi.encodePacked(
        ...
        '"image": "', _getImage(bg1,bg2,hair,skin,dress,faceIndexStr,armsIndexStr,lipsIndexStr), '"}'))))
    );
  }

  function _getImagePart1(string memory bg1, string memory bg2, string memory hair, string memory skin) internal pure returns (string memory) {
  ...
  function _getImage(string memory bg1, string memory bg2, string memory hair, string memory skin, string memory dress, string memory faceIndexStr, string memory armsIndexStr, string memory lipsIndexStr) internal view returns (string memory) {
    string memory svgBase64Encoded = Base64.encode(bytes(string(abi.encodePacked(
      _getImagePart1(bg1,bg2,hair,skin),
      _getImagePart2(dress, faceIndexStr,armsIndexStr,lipsIndexStr)
    ))));
