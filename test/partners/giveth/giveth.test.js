...
describe(".giveth minting contract", function () {
  let tldContract;
  const tldName = ".giveth";
  const tldSymbol = ".GIVETH";
  const tldPrice = 0;
  const tldRoyalty = 0;
  ...
  beforeEach(async function () {
    ...
    // Minter contract 
    const minterCode = await ethers.getContractFactory("GivethMinter");
  ...
  it("should mint two 5+ char domains", async function () {
    ...
    expect(mdResult1.name).to.equal("user1.giveth");
    ...
    expect(mdResult2.name).to.equal("user1second.giveth");
