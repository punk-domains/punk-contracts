// Run tests:
// npx hardhat test test/partners/ppl/ppl.test.js 
...
describe(".ppl minting contract", function () {
  let tldContract;
  const tldName = ".ppl";
  const tldSymbol = ".PPL";
  const tldPrice = 0;
  ...
  const nftName1 = "Dishes Collection #1";
  const nftSymbol1 = "DISH1";
  const nftName2 = "Dishes Collection #2";
  const nftSymbol2 = "DISH2";
  const nftName3 = "Dishes Collection #3";
  const nftSymbol3 = "DISH3";
  ...
  beforeEach(async function () {
    ...
    // deploy custom metadata contract
    const customMetadata = await ethers.getContractFactory("PplMetadata");
    metadataContract = await customMetadata.deploy();
    ...
    nftContract3 = await Erc721Contract.deploy(nftName3, nftSymbol3);

    // Minter contract
    const minterCode = await ethers.getContractFactory("PplMinter");
    mintContract = await minterCode.deploy(
      tldContract.address, // TLD address
      nftContract1.address, nftContract2.address, nftContract3.address, // NFT addresses
  ...
  it("should mint two 5+ char domains", async function () {
    ...
    // can user mint?
    const user1Nft1BalanceBefore1 = await nftContract1.balanceOf(user1.address);
    expect(user1Nft1BalanceBefore1).to.equal(0);
    const user1Nft2BalanceBefore1 = await nftContract2.balanceOf(user1.address);
    expect(user1Nft2BalanceBefore1).to.equal(0);
    const user1Nft3BalanceBefore1 = await nftContract3.balanceOf(user1.address);
    expect(user1Nft3BalanceBefore1).to.equal(0);

    const canUser1MintBefore1 = await mintContract.canUserMint(user1.address);
    expect(canUser1MintBefore1).to.be.false;

    // should fail at minting before having required NFTs
    await expect(mintContract.connect(user1).mint(
      "123456", // domain name (without TLD)
      user1.address, // domain holder
      ethers.constants.AddressZero, // no referrer in this case
    ...
    // user1 gets required NFTs

    await nftContract1.mint(user1.address);
    await nftContract2.mint(user1.address);
    await nftContract3.mint(user1.address);

    // can user mint now?
    const user1Nft1BalanceBefore2 = await nftContract1.balanceOf(user1.address);
    expect(user1Nft1BalanceBefore2).to.equal(1);
    const user1Nft2BalanceBefore2 = await nftContract2.balanceOf(user1.address);
    expect(user1Nft2BalanceBefore2).to.equal(1);
    const user1Nft3BalanceBefore2 = await nftContract3.balanceOf(user1.address);
    expect(user1Nft3BalanceBefore2).to.equal(1);

    const canUser1MintBefore2 = await mintContract.canUserMint(user1.address);
    expect(canUser1MintBefore2).to.be.true;
    ...
    expect(mdResult1.name).to.equal("user1.ppl");
    ...
    expect(mdResult2.name).to.equal("user1second.ppl");
    //console.log(mdResult1.image);
  ...
  it("should change royalty fee (only royalty fee updater)", async function () {
    ...
    expect(feeBefore).to.equal(2000);
    ...
    // cannot exceed 40%
    await expect(mintContract.connect(signer).changeRoyaltyFee(4100)).to.be.revertedWith('Cannot exceed 40%');
