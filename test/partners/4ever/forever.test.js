// Run tests:
// npx hardhat test test/partners/4ever/forever.test.js 
...
describe(".4ever minting contract", function () {
  let tldContract;
  const tldName = ".4ever";
  const tldSymbol = ".4EVER";
  const tldPrice = 0;
  const tldRoyalty = 0;
  const tldReferral = 0;
  ...
  beforeEach(async function () {
    ...
    // Minter contract
    const minterCode = await ethers.getContractFactory("ForeverMinter");
  ...
  it("should mint two 5+ char domains", async function () {
    ...
    expect(mdResult1.name).to.equal("user1.4ever");
    ...
    expect(mdResult2.name).to.equal("user1second.4ever");
    //console.log(mdResult1.image);
